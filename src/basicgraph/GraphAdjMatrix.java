...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		int jjjj = this.getNumVertices();
		int[][] cwy = new int[jjjj][jjjj];
		for (int i = 0; i < cwy.length; i ++) {
			for (int j = 0; j < cwy.length; j ++) {
				for(int k = 0; k<cwy.length; k++){
					cwy[i][j] += adjMatrix[i][k]*adjMatrix[k][j];
				}				
			}
		}
		List<Integer> cwy2 = new ArrayList<Integer>();
		for (int i=0; i<getNumVertices();i++){
			if (cwy[v][i]>0){
				int haha = cwy[v][i];
